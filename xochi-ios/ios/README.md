...
Final TestFlight project: `ios/build/testflight/Xochi.xcodeproj`. Build 3 now contains the companion learning encounter and updated spoken-direction bridge. Its final simulator build passed; the release archive is awaiting the user's macOS signing-key prompt. Latest successfully uploaded archive remains `ios/build/Xochi-0.1.0-2.xcarchive`. The build 1 candidate passed Apple's complete validation checks; both previously uploaded archive builds succeeded.
