...
## Companion Apple Intelligence contract (build 4)

`companion_voice.gd` uses `prepare_intelligence(locale)` while the player speaks/types to warm one empty fixed-prompt session; a request consumes it exactly once. Backgrounding and model unavailability discard it. It uses `intelligence_status(locale)` to obtain `available`, `disabled`, `not_ready`, `unsupported_device`, `unsupported_locale` or `unavailable`. This query never opens the microphone. `interpret_intent(text, locale, checkpoint, attempt, session)` returns through `interpretation(intent, language, checkpoint, attempt, session)`.

An anchored request-shape gate rejects non-guidance before inference. The Swift model uses `@Generable` enums for one finite intent (including rejection) and actual utterance language. An empty intent rejects a request; an English or uncertain utterance never becomes Spanish evidence merely because the recognizer was set to Spanish. The model receives neither the scene state nor its expected answer. Each natural direction has its own session after capture finishes; the old microphone terminal status cannot cancel that session. New input, stop, interruption and backgrounding cancel inference. Swift returns a rejection after five seconds; an independent six-second adapter watchdog keeps the UI usable if a native callback is missing.

Exact phrases bypass the model. Unsupported devices retain normal speech when their on-device recognizer is available, plus touch and typed short phrases. Pause exposes **Type a plan** even when mic permission was denied. Native recognition and natural-language availability are independent. Transcripts exceeding 240 characters are rejected without truncation, so a trailing negation or second command cannot be silently discarded.

The final host-model probe did not establish usable latency: with five seconds of preparation, all six inference-backed cases still hit the app's five-second deadline. Nine deterministic rejected-input cases and all 15 separate request-filter tests passed. Natural guidance is experimental; use the reproducible host probe with `--guard-only`, `--prewarm-seconds 5`, or `--timeout 15` to distinguish grammar checks, app-deadline behavior and extended semantic diagnosis. A Mac test is not an iPhone benchmark.
...
Final TestFlight project: `ios/build/testflight/Xochi.xcodeproj`. Version 0.1.0 (4) adds natural companion directions using Apple Intelligence. The latest verified distribution result is recorded in [the upload receipt](TESTFLIGHT-UPLOAD.md); physical model and microphone behavior remain separate from build validation.

For subsequent uploads, increment `application/version` in `export_presets.cfg`, then run `./ios/upload_testflight.sh`. This signs and uploads using the existing Xcode account and `ExportOptions.plist`, preserving a separate archive per version/build; no private signing material is stored in the repository.
