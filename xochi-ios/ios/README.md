...
Final TestFlight project: `ios/build/testflight/Xochi.xcodeproj`. Signed archive: `ios/build/Xochi-TestFlight.xcarchive`. This export includes the final scripts, narrowed privacy declarations and canal artwork on both native launch and engine startup. The earlier candidate passed Apple's complete validation checks; the final archive rebuild also succeeded.

Version 0.1.0 (1) uploaded successfully on 8 September 2026; Apple reported that the package was processing. See [upload receipt](TESTFLIGHT-UPLOAD.md). For subsequent uploads, increment `application/version` in `export_presets.cfg`, then run `./ios/upload_testflight.sh`. This explicitly signs and uploads using the Xcode account and `ExportOptions.plist`; no private signing material is stored in the repository. `ios/build/testflight-upload.log` records this session's upload result.
